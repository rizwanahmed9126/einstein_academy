...
import 'package:einstien_academy_flutter/models/cspicker_model.dart';
...
class _CSPickerWidgetState extends State<CSPickerWidget> {
 ...
 // String? _selectedCountry="${widget.selectedCountry}";
  ...
  @override
  void initState() {
    // locationData = Provider.of<BusinessProvider>(context, listen: false).geLocationListData;
    // for (var j = 0; j < locationData.length; j++) {
    //   if (locationData != null) {
    //     _selectedCountry = locationData[j]["countryValue"];
    //     _selectedCity = locationData[j]["cityValue"];
    //     _selectedState = locationData[j]["stateValue"];
    //   }
    // }
    getCounty();
  ...
  Future getState() async {
    ...
    var takeState = widget.flagState == CountryFlag.ENABLE ||
        ...
        ? response
            ...
            .where(
                (item) => item.emoji + "    " + item.name == widget.selectedCountry)
            .map((item) => item.state)
            .toList()
        : response
            .map((map) => Country.fromJson(map))
            .where((item) => item.name == widget.selectedCountry)
            .map((item) => item.state)
            .toList();
  ...
  void _onSelectedCountry(String? value) {
    ...
    setState(() {
      ...
      if (value != widget.selectedCountry) {
        _states!.clear();
        _cities!.clear();
      ...
      child: Padding(
        ...
        child: DropdownButtonHideUnderline(
          child: DropdownWithSearch(
            ...
            selectedItemStyle: widget.selectedCountry != null
            ...
            selected: widget.selectedCountry != null ? widget.selectedCountry : "Country",
            //selected: _selectedCountry != null ? _selectedCountry : "Country",
            //onChanged: (value) => _onSelectedCountry(value),
            onChanged: (value) {
              print("countryChanged $value $widget.selectedCountry");
              if (value != null) {
...
class CSPickerWidget1 extends StatefulWidget {
  ...
  @override
  _CSPickerWidget1State createState() => _CSPickerWidget1State();
}

class _CSPickerWidget1State extends State<CSPickerWidget1> {
  bool isActive = false;
  List<String>? _cities = [];
  List<String>? _country = [];
  List<String>? _states = [];
